...
order by 1,2 limit 100000;

-- Total Cases vs Population

select Location,Date_Format(date,"%e/%c/%y") as date,Max(total_cases),Population, Max((total_cases/Population)*100) as Infected
from Coviddeaths
...
select Location,Date_Format(date,"%e/%c/%y") as date,Max(total_deaths),Population, Max((total_deaths/Population)*100) as Dead
from Coviddeaths
group by location,population
order by Dead desc;
...
select * from Percent_population_Vaccinated limit 100000;
